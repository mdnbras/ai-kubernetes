...
const expectedName = "ai-kubernetes";
const errors = [];
...
if (manifest.name !== expectedName || !manifest.version) errors.push("Manifesto Kubernetes inválido.");
if (manifest.entryAgent !== `specialists/${expectedName}/orchestrator`) errors.push("Entry agent Kubernetes inválido.");
for (const companion of ["ai-aws", "ai-terraform"]) if (!(manifest.companions ?? []).includes(companion)) errors.push(`Companion ausente: ${companion}`);
